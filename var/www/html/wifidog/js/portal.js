...
//	var url_cgi = base_url+ 'test.cgi?type='+type+'&username='+name;
	if(type == 0)
	{
		var url_cgi = 'http://'+document.location.hostname+':8080/node/video';
	...
	if(type==1)
	{
		var url_cgi = 'http://'+document.location.hostname+':8080/node/live';
		$.get(url_cgi, function(data) {
			var obj=JSON.parse(data);
			wrapData(obj);
		});
...
//	var data;
//  if (type == 0) {
//		data = '{"code":"1000","data":{"powers":0,"type":0,"movieList":[{"imagePath":"BornInChina.jpg","name":"BornInChina","vedioPath":"BornInChina"},{"imagePath":"十二公民.jpg","name":"十二公民","vedioPath":"十二公民"},{"imagePath":"无人�?jpg","name":"无人�?,"vedioPath":"无人�?}]},"msg":"ok"}';
//	} else {
...
function wrapData(data) {
	...
	$.each(data.movieList, function(index, val) {
		...
		if(data.type==1)
			img.setAttribute("src", base_url + "/image/" + val.imagePath);
		else if(data.type==0)
			img.setAttribute("src", base_url + "/media/" + val.imagePath);
...
function getVideoPath(type, path) {
	if (type == 0) {
		...
		return base_url+"/hls/"+path+"?t="+Date.parse(new Date());

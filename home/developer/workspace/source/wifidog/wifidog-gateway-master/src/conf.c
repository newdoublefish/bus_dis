...
static const struct {
	...
	"YunHostname",oAuthServYunHostname},{
	"YunPort",oAuthServYunPort},{
...
parse_auth_server(FILE * file, const char *filename, int *linenum)
{
        ...
        if (strlen(p1) > 0) {
            ...
            *p2 = '\0';
            ...
            case oAuthServHostname:
                ...
                host = safe_strdup(p2);
                break;
			case oAuthServYunHostname:
				if (NULL != yunHost)
                    free(yunHost);
                yunHost = safe_strdup(p2);
                break;
			case oAuthServYunPort:
				if (NULL != yunPort)
                    free(yunPort);
                yunPort = safe_strdup(p2);
                break;
            case oAuthServPath:
                free(path);
                path = safe_strdup(p2);
	...
	new->authserv_yunhostname = yunHost;
	new->authserv_YunPort= yunPort;
    ...
    debug(LOG_INFO, "Auth server added and yunhostname is %s,yunPort is %s\n",yunHost,yunPort);
}

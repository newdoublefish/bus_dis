...
iptables_load_ruleset(const char *table, const char *ruleset, const char *chain)
{
    ...
    debug(LOG_INFO, "Load ruleset %s into table %s, chain %s", ruleset, table, chain);
    ...
    debug(LOG_INFO, "Ruleset %s loaded into table %s, chain %s", ruleset, table, chain);
}
#endif
void
...
iptables_fw_init(void)
{
    ...
    int proxy_port;
    fw_quiet = 0;
	//默认got_authdown_ruleset为NULL因为配置文件没有设置
    ...
    iptables_do_command("-t mangle -N " CHAIN_TO_INTERNET);
    iptables_do_command("-t mangle -N " CHAIN_GLOBAL);
    iptables_do_command("-t mangle -N " CHAIN_UNKNOWN);
    iptables_do_command("-t mangle -N " CHAIN_AUTHSERVERS);
    if (got_authdown_ruleset)
        iptables_do_command("-t mangle -N " CHAIN_AUTH_IS_DOWN);

    /* Assign links and rules to these new chains */
	/*插入规则到第一条，原本第一条的规则往下移*/
	...
	iptables_do_command("-t mangle -I FORWARD -i %s -j " CHAIN_TO_INTERNET, config->gw_interface);

	iptables_do_command("-t mangle -A " CHAIN_TO_INTERNET " -m mark --mark 0x%u -j ACCEPT", FW_MARK_KNOWN);
	iptables_do_command("-t mangle -A " CHAIN_TO_INTERNET " -j " CHAIN_UNKNOWN);

	iptables_do_command("-t mangle -A " CHAIN_UNKNOWN " -j " CHAIN_AUTHSERVERS);
	iptables_do_command("-t mangle -A " CHAIN_AUTHSERVERS " -d %s -j ACCEPT", "120.25.227.65");

	iptables_do_command("-t mangle -A " CHAIN_UNKNOWN " -j " CHAIN_GLOBAL);
	iptables_do_command("-t mangle -A " CHAIN_GLOBAL " -p tcp -m string --string %s --algo bm -j ACCEPT", "openapi.alipaydev.com");
	iptables_do_command("-t mangle -A " CHAIN_GLOBAL " -p tcp -m string --string %s --algo bm -j ACCEPT", "as.alipayobjects.com");
	iptables_do_command("-t mangle -A " CHAIN_GLOBAL " -p tcp -m string --string %s --algo bm -j ACCEPT", "a.alipayobjects.com");
	iptables_do_command("-t mangle -A " CHAIN_GLOBAL " -p tcp -m string --string %s --algo bm -j ACCEPT", "tscenter.alipay.com");
		//debug版本，用于连接云服务器
	iptables_do_command("-t mangle -A " CHAIN_GLOBAL " -p tcp --dport 22 -j ACCEPT");
	//iptables_do_command("-t mangle -A " CHAIN_UNKNOWN " -j DROP");
#endif
    ...
    /* Assign links and rules to these new chains */
    	//将访问外网和访问192.168.0.5的数据包做处理，但不对访问0网段其他ip的做处理，跳转到CHAIN_OUTGOING
    iptables_do_command("-t nat -A PREROUTING -i %s -j " CHAIN_OUTGOING, config->gw_interface);
		//CHAIN_OUTGOING对访问本地192.168.0.5的数据包做处理，跳转到CHAIN_TO_ROUTER
    iptables_do_command("-t nat -A " CHAIN_OUTGOING " -d %s -j " CHAIN_TO_ROUTER, config->gw_address);
		//debug版本，用于连接本地服务器
	iptables_do_command("-t nat -A " CHAIN_TO_ROUTER " -p tcp --dport 22 -j ACCEPT");
    	//对带有标记FW_MARK_KNOWN的数据包放行ACCEPT
	iptables_do_command("-t nat -A " CHAIN_TO_ROUTER " -m mark --mark 0x%u -j ACCEPT", FW_MARK_KNOWN);
	//iptables_do_command("-t nat -A " CHAIN_TO_INTERNET " -m mark --mark 0x%u -j ACCEPT", FW_MARK_PROBATION);
		//对带有标记FW_MARK_JUST_PORTAL的访问8080的数据包放行ACCEPT
	iptables_do_command("-t nat -A " CHAIN_TO_ROUTER " -m mark --mark 0x%u -p tcp  --dport 8080 -j ACCEPT", FW_MARK_JUST_PORTAL);	
		//对访问192.168.0.5非80端口的重定向到2060 ACCEPT
	iptables_do_command("-t nat -A " CHAIN_TO_ROUTER " -p tcp ! --dport 80 -j REDIRECT --to-ports %d", gw_port);
	iptables_do_command("-t nat -A " CHAIN_TO_ROUTER " -j ACCEPT");
		//对访问外网的跳转到CHAIN_TO_INTERNET
	...
	iptables_do_command("-t nat -A " CHAIN_AUTHSERVERS " -d %s -j ACCEPT", auth_server->authserv_yunhostname);
	iptables_do_command("-t nat -A " CHAIN_AUTHSERVERS " -p tcp --dport 22 -j ACCEPT");
	
    iptables_do_command("-t nat -A " CHAIN_UNKNOWN " -j " CHAIN_GLOBAL);
	iptables_do_command("-t nat -A " CHAIN_GLOBAL " -d %s -j ACCEPT", "openapi.alipaydev.com");
	iptables_do_command("-t nat -A " CHAIN_GLOBAL " -d %s -j ACCEPT", "as.alipayobjects.com");
	iptables_do_command("-t nat -A " CHAIN_GLOBAL " -d %s -j ACCEPT", "a.alipayobjects.com");
	iptables_do_command("-t nat -A " CHAIN_GLOBAL " -d %s -j ACCEPT", "tscenter.alipay.com");
		//debug版本，用于连接云服务器
    ...
    iptables_do_command("-t nat -A " CHAIN_UNKNOWN " -p tcp  -j REDIRECT --to-ports %d", gw_port);
    ...
    iptables_do_command("-t filter -A " CHAIN_TO_INTERNET " -m mark --mark 0x%u -j ACCEPT", FW_MARK_KNOWN);
    iptables_do_command("-t filter -A " CHAIN_TO_INTERNET " -j " CHAIN_UNKNOWN);

	iptables_do_command("-t filter -A " CHAIN_UNKNOWN " -j " CHAIN_AUTHSERVERS);
	iptables_do_command("-t filter -A " CHAIN_AUTHSERVERS " -d %s -j ACCEPT", "120.25.227.65");

    iptables_do_command("-t filter -A " CHAIN_UNKNOWN " -j " CHAIN_GLOBAL);
	iptables_do_command("-t filter -A " CHAIN_GLOBAL " -p tcp -m string --string %s --algo bm -j ACCEPT", "openapi.alipaydev.com");
	iptables_do_command("-t filter -A " CHAIN_GLOBAL " -p tcp -m string --string %s --algo bm -j ACCEPT", "as.alipayobjects.com");
	iptables_do_command("-t filter -A " CHAIN_GLOBAL " -p tcp -m string --string %s --algo bm -j ACCEPT", "a.alipayobjects.com");
	iptables_do_command("-t filter -A " CHAIN_GLOBAL " -p tcp -m string --string %s --algo bm -j ACCEPT", "tscenter.alipay.com");
		//debug版本，用于连接云服务器
	iptables_do_command("-t filter -A " CHAIN_GLOBAL " -p tcp --dport 22 -j ACCEPT");
...
iptables_fw_destroy(void)
{
	...
	iptables_fw_destroy_mention("mangle", "FORWARD", CHAIN_TO_INTERNET);
    if (got_authdown_ruleset)
	...
	iptables_do_command("-t mangle -F ", CHAIN_TO_INTERNET);
    iptables_do_command("-t mangle -F ", CHAIN_GLOBAL);
	iptables_do_command("-t mangle -F ", CHAIN_UNKNOWN);
	iptables_do_command("-t mangle -F ", CHAIN_AUTHSERVERS);
	iptables_do_command("-t mangle -X ", CHAIN_AUTHSERVERS);
	iptables_do_command("-t mangle -X ", CHAIN_TO_INTERNET);
	iptables_do_command("-t mangle -X ", CHAIN_GLOBAL);
	iptables_do_command("-t mangle -X ", CHAIN_UNKNOWN);
...
iptables_fw_counters_update(void)
{
	...
	debug(LOG_WARNING,"action:%s",script);
    ...
    while (output && !(feof(output))) {
        ...
        if (2 == rc && EOF != rc) {
            ...
            if ((p1 = client_list_find_by_ip(ip))) {
				debug(LOG_INFO, "Read outgoing traffic for %s: Bytes=%llu", ip, counter + p1->counters.outgoing_history);
				//流量计算公式:
				//outgoing = outgoing'+outgoing_delta = counter + outgoing_history
				//如果增量为0，那么不更新数据。
				//上次累计流量小于(当前统计+历史流量 = 总流量)
				//重启后||刷新iptables规则后，应当保证outgoing == outgoing_history
                if (p1->counters.outgoing < (counter + p1->counters.outgoing_history)) {
					//流量增量
    ...
    while (output && !(feof(output))) {
        ...
        if (2 == rc && EOF != rc) {
            ...
            LOCK_CLIENT_LIST();
            if ((p1 = client_list_find_by_ip(ip))) {
				debug(LOG_INFO, "Read incoming traffic for %s: Bytes=%llu", ip, counter + p1->counters.incoming_history);
                if (p1->counters.incoming < (counter + p1->counters.incoming_history)) {
...
void fw_alipay(void *arg){
	...
	while(1){
		...
		debug(LOG_WARNING, "FRESH ALIPAY IPTABLES!!");
		iptables_do_command("-t nat -F " CHAIN_GLOBAL);
		iptables_do_command("-t nat -A " CHAIN_GLOBAL " -d %s -j ACCEPT", "openapi.alipaydev.com");
		iptables_do_command("-t nat -A " CHAIN_GLOBAL " -d %s -j ACCEPT", "as.alipayobjects.com");
		iptables_do_command("-t nat -A " CHAIN_GLOBAL " -d %s -j ACCEPT", "a.alipayobjects.com");

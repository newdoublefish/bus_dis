 ...
 */
t_client *
client_list_add(const char *ip, const char *mac, const char *token, const char *name,const char *passwd)
{
    t_client *curclient;
	...
	curclient->name = safe_strdup(name);
	curclient->passwd = safe_strdup(passwd);
	curclient->fw_connection_state = -1;
	curclient->access = -1;
    ...
    curclient->counters.login_time = curclient->counters.last_updated = time(NULL);
...
client_dup(const t_client * src)
{
	...
	new->passwd= safe_strdup(src->passwd);
	new->access = (src->access);
	new->old_access = (src->old_access);
	new->fw_connection_state = (src->fw_connection_state);
	new->old_state = (src->old_state);
	...
	new->counters.login_time = src->counters.login_time;
    new->next = NULL;
...
int try_client(const char *ip, const char *mac){
	t_client *ptr;
	LOCK_CLIENT_LIST();
	ptr = client_list_find(ip,mac);
	if(ptr == NULL){
		debug(LOG_ERR, "no this client");
		UNLOCK_CLIENT_LIST();
		return -1;
	}
	UNLOCK_CLIENT_LIST();
...
int set_state_access(const char *ip,int state,int access){
	t_client *client;
	LOCK_CLIENT_LIST();
	client = client_list_find_by_ip(ip);
	if(client == NULL){
		debug(LOG_ERR, "no this client to fresh");
		UNLOCK_CLIENT_LIST();
		return -1;
	}
	client->old_state = client->fw_connection_state;
	client->fw_connection_state = state;
	client->old_access = client->access;
	client->access = access;
	debug(LOG_INFO, "after set_state_access,new state[%d],old state[%d]",client->fw_connection_state,client->old_state);
	UNLOCK_CLIENT_LIST();
	return 0;
}

int get_state_access(const char *ip,int *state,int *access){
	t_client *client;
	LOCK_CLIENT_LIST();
	client = client_list_find_by_ip(ip);
	if(client == NULL){
		debug(LOG_INFO, "no this client to fresh");
		UNLOCK_CLIENT_LIST();
		return -1;
	}
	*state = client->fw_connection_state;
...
int set_state(const char *ip,int state){
	t_client *client;
	LOCK_CLIENT_LIST();
	client = client_list_find_by_ip(ip);
	if(client == NULL){
...
int get_2access(const char *ip,int *access, int *old_access){
	t_client *client;
	...
	*access = client->access;
	*old_access = client->old_access;
	UNLOCK_CLIENT_LIST();
	return 0;
}


char *get_username(const char *ip){
	t_client *client;
	char *username;
	LOCK_CLIENT_LIST();
	client = client_list_find_by_ip(ip);
...
t_client *client_dup_by_ip(const char *ip){
	t_client *client;
	LOCK_CLIENT_LIST();
	client = client_list_find_by_ip(ip);
	if(client == NULL){
 ...
 */
int client_list_delete(const char *ip)
{
	t_client *client;
	LOCK_CLIENT_LIST();
	client = client_list_find_by_ip(ip);
